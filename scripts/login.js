...
const signUp = () => {
  if (checkEmptyInputs("signup")) {
    ...
    for (let i = 0; i < allUsers.length; i++) {
      if (
        newUser.username.toLowerCase() == allUsers[i].username.toLowerCase()
      ) {
        eligible = false;
        break;
      } else {
...
const login = () => {
  if (checkEmptyInputs("login")) {
    ...
    for (let i = 0; i < allUsers.length; i++) {
      if (
        oldUser.username.toLowerCase() == allUsers[i].username.toLowerCase() &&
        oldUser.password === allUsers[i].password
      ) {
        error = false;
