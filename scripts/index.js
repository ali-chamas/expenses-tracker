...
const getTotal = async () => {
  ...
  for (let i = 0; i < finances.length; i++) {
    if (finances[i].type == "expense") {
      totalExpenses += await convertAmount(
        finances[i].currency,
        activeCurrency.code,
        finances[i].amount
      );
    } else {
      totalIncomes += await convertAmount(
        finances[i].currency,
        activeCurrency.code,
        finances[i].amount
      );
...
const setAmounts = () => {
  ...
  let types = [
    {
      title: "expenses",
      amount: formatDouble(totalExpenses),
      icon: '<i class="fa-solid fa-arrow-trend-down"></i>',
    },
    {
      title: "incomes",
      amount: formatDouble(totalIncomes),
      icon: '<i class="fa-solid fa-arrow-trend-up"></i>',
    },
    {
      title: "balance",
      amount: formatDouble(totalIncomes - totalExpenses),
...
const setFinances = async (array) => {
  ...
  for (let i = 0; i < array.length; i++) {
    financesContainer.innerHTML += ` <div class="w-full flex justify-between align-center single-finance" onClick='openInfo(${
      array[i].id
    })'>
...
const changeActiveCurrency = async (symbol) => {
  ...
  currencies.forEach((curr) => {
    if (curr.symbol == symbol) activeCurrency = curr;
  });
...
const filterFinances = async (value) => {
  ...
  if (value == "expenses") {
    filteredFinances = finances.filter((finance) => finance.type == "expense");
  } else if (value == "incomes") {
    filteredFinances = finances.filter((finance) => finance.type == "income");
  } else {
...
const formAdders = () => {
  expenseName.addEventListener(
    "change",
    (e) => (expense.name = e.target.value)
  );
  expenseAmount.addEventListener(
    "change",
    (e) => (expense.amount = e.target.value)
  );
  incomeName.addEventListener("change", (e) => (income.name = e.target.value));
  incomeAmount.addEventListener(
    "change",
    (e) => (income.amount = e.target.value)
  );
  selectExpense.addEventListener(
    "change",
    (e) => (expense.currency = e.target.value)
  );
  selectIncome.addEventListener(
    "change",
    (e) => (income.currency = e.target.value)
  );
...
const addFinance = (finance) => {
  ...
  users.map((u) => {
    if (u.username == user.username) u.finances = user.finances;
  });
...
const openInfo = async (id) => {
  ...
  infoPopup.innerHTML = `<div class="info-popup flex column center">
                <div class="flex gap align-center">
                    ...
                    )}<span>${activeCurrency.symbol}</span></p>
                </div>
                <div class="flex gap">
                    <button class="btn-style bg-secondary"  onClick='openEditPopup()'>Edit <i class="fa-solid fa-pen"></i></button>
                    <button class="btn-style bg-danger text-white" onClick='deleteFinance(${
                      finance.id
  ...
  editCurrency.addEventListener(
    "change",
    (e) => (newCurrency = e.target.value)
  );
  confirmEdit.addEventListener("click", () =>
    editFinance(finance.id, newName, newAmount, newCurrency)
  );
...
const app = async () => {
  ...
  currencyBtns.forEach((btn) => {
    btn.addEventListener(
      "click",
      async () => await changeActiveCurrency(btn.innerHTML)
    );
  });
